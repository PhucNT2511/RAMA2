...
class Trainer:
    ...
    def train_one_epoch(self, p_value=None):
        ...
        for batch_idx, (orig_inputs, targets) in enumerate(pbar):
            # print(type(inputs), type(targets)) # Original line, can be removed or kept for debugging
            ...
            if self.args and self.args.adversarial_training:
                ...
                self.model.train() # Switch back to train mode for training step
            
            self.optimizer.zero_grad()
            # Train on current_batch_inputs (either original or adversarial), using the same p_value for RAMA
            outputs = self.model.forward(current_batch_inputs, p_value=p_value)
...
def get_experiment_name(args: argparse.Namespace) -> str:
    ...
    if args.use_rama:
        exp_name += f"_{args.bernoulli_values}"
