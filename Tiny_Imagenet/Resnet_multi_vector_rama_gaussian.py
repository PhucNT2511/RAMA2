...
class Trainer:
    ...
    def train_one_epoch(self, lambda_value=None):
        ...
        for batch_idx, (orig_inputs, targets) in enumerate(pbar):
            ...
            if self.args and self.args.adversarial_training:
                ...
                # Use the provided lambda_value for attack generation
                ...
                current_batch_inputs = adv_inputs # Use adversarial inputs for this batch
                self.model.train() # Switch back to train mode
            
            self.optimizer.zero_grad()
            # Train on current_batch_inputs (either original or adversarial), using the same lambda_value for RAMA
    ...
    def train(self, epochs, start_epoch=0):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            # Train with best sigma_p
            train_loss, train_acc = self.train_one_epoch(lambda_value=self.best_sigma_p)
            ...
            if (self.use_rama and self.use_hyperparameter_optimization and
                ...
                if bayesian_score >= test_acc:
                    self.best_sigma_p = lambda_value
                    logger.info(f"Updated best sigma_p: {self.best_sigma_p:.4f} with accuracy: {bayesian_score:.2f}%")
                    ...
                    if lambda_min_distance < lambda_max_distance:
                        new_max = min(self.best_sigma_p * 1.5, self.bayes_opt_config["lambda_max"] * 2)
                    ...
                    # If closer to max bound, expand lower bound.
                        ...
                        new_min = max(self.best_sigma_p * 0.5, self.bayes_opt_config["lambda_min"] * 0.5)
                        self.bayesian_optimizer.set_bounds(
            ...
            if self.use_rama and self.use_hyperparameter_optimization:
                logger.info(f"Current sigma_p value: {self.best_sigma_p:.6f}")
            ...
            if self.neptune_run:
                ...
                if self.use_rama and self.use_hyperparameter_optimization:
                    self.neptune_run["RAMA_LAMBDA"].append(self.best_sigma_p)
            ...
            if self.writer:
                ...
                if self.use_rama and self.use_hyperparameter_optimization:
                    self.writer.add_scalar("RAMA_LAMBDA", self.best_sigma_p, epoch)
            ...
            self.save_checkpoint({
                ...
                "best_sigma_p": self.best_sigma_p,
            }, is_best)
...
def get_experiment_name(args: argparse.Namespace) -> str:
    ...
    exp_name = "ResNet18_TinyImageNet"
