...
# Define a replacement function for the deprecated zero_gradients
...
class FABAttack():
    ...
    def projection_linf(self, points_to_project, w_hyperplane, b_hyperplane):
        ...
        u = torch.arange(0, w.shape[0]).to(self.device)
        ...
        lb = torch.zeros(c2.shape[0]).to(self.device)
        ub = torch.ones(c2.shape[0]).to(self.device) * (w.shape[1] - 1)
        nitermax = torch.ceil(torch.log2(torch.tensor(w.shape[1]).float()))
        counter2 = torch.zeros(lb.shape).long().to(self.device)
    ...
    def projection_l1(self, points_to_project, w_hyperplane, b_hyperplane):
        ...
        u = torch.arange(0, w.shape[0]).unsqueeze(1).to(self.device)
        u2 = torch.arange(0, w.shape[1]).repeat(w.shape[0], 1).to(self.device)
        c6 = (w < 0).float()
        d = (-t + c6) * (w != 0).float()
        ...
        counter = 0
        lb = torch.zeros(c2.shape[0]).to(self.device)
        ub = torch.ones(c2.shape[0]).to(self.device) * (s.shape[1])
