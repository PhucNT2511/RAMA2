...
def eval_at_metrics(model, test_loader, neptune_run=None):
    ...
    print(f"AT_fgsm_loss: {AT_fgsm_loss}, AT_fgsm_acc: {AT_fgsm_acc}")
    print(f"AT_pgd_loss_10: {AT_pgd_loss_10}, AT_pgd_acc_10: {AT_pgd_acc_10}")
    print(f"AT_pgd_loss_20: {AT_pgd_loss_20}, AT_pgd_acc_20: {AT_pgd_acc_20}")
    print(f"AT_pgd_loss_50: {AT_pgd_loss_50}, AT_pgd_acc_50: {AT_pgd_acc_50}")
    print(f"AT_CW_loss_20: {AT_CW_loss_20}, AT_pgd_cw_acc_20: {AT_pgd_cw_acc_20}")
    print(f"AT_models_test_loss: {AT_models_test_loss}, AT_models_test_acc: {AT_models_test_acc}")
    if neptune_run:
        # Log AT metrics to neptune
        neptune_run["final_metrics/fgsm_accuracy"] = AT_fgsm_acc
        neptune_run["final_metrics/fgsm_loss"] = AT_fgsm_loss
        neptune_run["final_metrics/pgd10_accuracy"] = AT_pgd_acc_10
        neptune_run["final_metrics/pgd10_loss"] = AT_pgd_loss_10
        neptune_run["final_metrics/pgd20_accuracy"] = AT_pgd_acc_20
        neptune_run["final_metrics/pgd20_loss"] = AT_pgd_loss_20
        neptune_run["final_metrics/pgd50_accuracy"] = AT_pgd_acc_50
        neptune_run["final_metrics/pgd50_loss"] = AT_pgd_loss_50
        neptune_run["final_metrics/pgd_cw_accuracy"] = AT_pgd_cw_acc_20
        neptune_run["final_metrics/pgd_cw_loss"] = AT_CW_loss_20
        neptune_run["final_metrics/test_accuracy"] = AT_models_test_acc
        neptune_run["final_metrics/test_loss"] = AT_models_test_loss
...
def main():
    ...
    # Modify the logging configuration to properly setup the logger
    logging.basicConfig(
    ...
    for epoch in range(args.epochs):
        ...
        for i, (X, y) in enumerate(pbar):
            ...
            # Update tqdm progress bar with current loss value
            pbar.set_postfix({'loss': f'{loss.item():.4f}', 'acc': f'{train_acc/max(1, train_n):.4f}'})
        ...
        if neptune_run:
            neptune_run["training/epoch"].log(epoch)
            neptune_run["training/epoch_time"].log(epoch_time)
            neptune_run["training/learning_rate"].log(lr)
            neptune_run["training/loss"].log(train_loss / train_n)
            neptune_run["training/accuracy"].log(train_acc / train_n)
            neptune_run["training/init_loss"].log(init_train_loss / train_n)
            neptune_run["training/init_accuracy"].log(init_train_acc / train_n)
        ...
        if neptune_run:
            neptune_run["evaluation/test_loss"].log(test_loss)
            neptune_run["evaluation/test_accuracy"].log(test_acc)
            neptune_run["evaluation/pgd_loss"].log(pgd_loss)
            neptune_run["evaluation/pgd_accuracy"].log(pgd_acc)
        ...
        if best_result <= pgd_acc:
            ...
            # Log best result to neptune
            if neptune_run:
                neptune_run["best_metrics/pgd_accuracy"] = pgd_acc
                neptune_run["best_metrics/test_accuracy"] = test_acc
                neptune_run["best_metrics/epoch"] = epoch

    if best_model is not None:
        eval_at_metrics(best_model, test_loader, neptune_run)
    torch.save(model_test.state_dict(), os.path.join(output_path, 'final_model.pth'))
    
    # Log training history
    if neptune_run:
        for i, (clean_acc, pgd_acc) in enumerate(zip(epoch_clean_list, epoch_pgd_list)):
            neptune_run["history/clean_accuracy"].log(clean_acc, step=i)
            neptune_run["history/pgd_accuracy"].log(pgd_acc, step=i)
        
        neptune_run["training_curves/clean_accuracy"] = epoch_clean_list
        neptune_run["training_curves/pgd_accuracy"] = epoch_pgd_list
        neptune_run["training_curves/init_loss"] = init_loss
        neptune_run["training_curves/init_accuracy"] = init_acc
        neptune_run["training_curves/final_loss"] = final_loss
        neptune_run["training_curves/final_accuracy"] = final_acc
