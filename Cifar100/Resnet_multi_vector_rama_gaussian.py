    ...
    def train_one_epoch(self, lambda_value=None):
        ...
        for batch_idx, (inputs, targets) in enumerate(pbar):
            ...
            if self.args and self.args.adversarial_training:
                ...
                current_mu_for_at = lambda_value # This is self.best_mu from the train loop
                inputs_for_attack = inputs.clone().detach()

                attack_model_wrapper_at = lambda imgs_for_attack: self.model.forward(imgs_for_attack, lambda_value=current_mu_for_at)
                ...
                outputs = self.model.forward(adv_inputs, lambda_value=current_mu_for_at)
    ...
    def evaluate_with_metrics(self, lambda_value=None, epoch=None, test_acc=None, total_epochs=None):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, targets) in enumerate(pbar_eval):
                ...
                current_mu_for_eval = lambda_value if self.use_rama else None
                outputs, before_features, after_features = self.model.forward_with_features(inputs, current_mu_for_eval)
                ...
                if test_acc > self.best_acc and (epoch % 15 == 0 or epoch == total_epochs - 1):
                    ...
                    if self.args and self.args.eval_fgsm:
                        ...
                        outputs_fgsm = self.model.forward(adv_images_fgsm, mu_value=current_mu_for_eval)
                    ...
                    if self.args and self.args.eval_pgd:
                        ...
                        outputs_pgd = self.model.forward(adv_images_pgd, mu_value=current_mu_for_eval)
    ...
    def train(self, epochs, start_epoch=0):
        ...
        if self.use_rama and self.use_hyperparameter_optimization:
            if start_epoch == 0:
                ...
                self.best_mu, _, _ = self.optimize_lambda()
            else:
                ...
                if self.bayesian_optimizer.max:
                    self.best_mu = self.bayesian_optimizer.max["params"]["lambda_value"]
                    logger.info(f"Loaded best lambda from previous run: {self.best_mu:.6f}")
                else:
        ...
        for epoch in range(start_epoch, epochs):
            ...
            # Train with best p
            train_loss, train_acc = self.train_one_epoch(lambda_value=self.best_mu)
            
            # Basic evaluation
            test_loss, test_acc = self.evaluate(lambda_value=self.best_mu)
            
            # Detailed evaluation with feature metrics (once every 5 epochs to save time)
            metrics = self.evaluate_with_metrics(lambda_value=self.best_mu, epoch=epoch, test_acc=test_acc, total_epochs=epochs)
            ...
            if (self.use_rama and self.use_hyperparameter_optimization and
                ...
                if bayesian_score >= test_acc:
                    self.best_mu = lambda_value
                    logger.info(f"Updated best lambda: {self.best_mu:.4f} with accuracy: {bayesian_score:.2f}%")
                    ...
                    if lambda_min_distance < lambda_max_distance:
                        new_max = min(self.best_mu * 1.5, self.bayes_opt_config["lambda_max"] * 2)
                    ...
                    # If closer to max bound, expand lower bound.
                        ...
                        new_min = max(self.best_mu * 0.5, self.bayes_opt_config["lambda_min"] * 0.5)
                        self.bayesian_optimizer.set_bounds(
            ...
            if self.use_rama and self.use_hyperparameter_optimization:
                logger.info(f"Current lambda value: {self.best_mu:.6f}")
            ...
            if self.neptune_run:
                ...
                if self.use_rama and self.use_hyperparameter_optimization:
                    self.neptune_run["RAMA_LAMBDA"].append(self.best_mu)
            ...
            if self.writer:
                ...
                if self.use_rama and self.use_hyperparameter_optimization:
                    self.writer.add_scalar("RAMA_LAMBDA", self.best_mu, epoch)
...
def parse_args():
    ...
    parser.add_argument('--use-hyperparameter-optimization', action='store_true', help='whether to use Bayesian optimization for mu-value')
