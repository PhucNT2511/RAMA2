...
class Trainer:
    ...
    def train_one_epoch(self, p_value=None):
        ...
        for batch_idx, (inputs, targets) in enumerate(pbar):
            ...
            if self.args and self.args.adversarial_training:
                ...
                outputs = self.model.forward(adv_inputs, p_value=current_p_for_at)
    ...
    def evaluate_with_metrics(self, p_value=None, epoch=None):
        """
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inputs, targets) in enumerate(pbar_eval):
                ...
                current_p_for_eval = p_value if self.use_rama else None
                outputs, before_features, after_features = self.model.forward_with_features(inputs, current_p_for_eval)
                ...
                if self.args and self.args.eval_fgsm:
                    ...
                    outputs_fgsm = self.model.forward(adv_images_fgsm, p_value=current_p_for_eval)
                ...
                if self.args and self.args.eval_pgd:
                    ...
                    outputs_pgd = self.model.forward(adv_images_pgd, p_value=current_p_for_eval)
    ...
    def train(self, epochs, start_epoch=0):
        ...
        for epoch in range(start_epoch, epochs):
            ...
            metrics = self.evaluate_with_metrics(p_value=self.best_p, epoch=epoch)
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(description='PyTorch CIFAR-100 Training with EfficientNet-B2 and Bernoulli RAMA Layers')
